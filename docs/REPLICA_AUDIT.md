...
> Alcance focalizado vigente: cerrar Pruebas y Guías según
> `docs/PRUEBAS_GUIAS_CIERRE.md`. Adaptaciones PIE mediante IA, simulación de
> estudiantes, calibración de Bloom e historial/versiones quedan registradas
> como **Próximamente** y no bloquean este cierre.
...
| Evaluaciones - Pruebas | `evaluaciones/pruebas/*`, `lib/pruebas.ts` | Hub + detalle + editor + aplicación/corrección nativa | Cierre focalizado: banco, importación/exportación y validación; PIE IA/simulación/Bloom/historial quedan Próximamente | Crear/editar/aplicar los 7 tipos, probar conflictos web/iOS, colegio principal/secundario y contenido desconocido |
| Evaluaciones - Guías | `evaluaciones/guias/*`, `lib/guias.ts`, `document-download.tsx` | Hub + detalle + editor + PDF nativo/formato institucional | Cierre focalizado: banco, IA general, Word, impresión y medios; historial queda Próximamente | Probar CRUD, currículo, PhotosPicker/Storage, 13 tipos, plantillas y exportaciones en dispositivo real |
...
- Las imágenes de bloques, opciones y colorear ya se seleccionan con `PhotosPicker`, se validan, redimensionan/comprimen bajo 8 MB y suben a `users/{uid}/evaluaciones/guias/{guiaId}/...` mediante Firebase Storage con progreso. Tras guardar y releer la versión canónica, iOS elimina únicamente rutas propias del documento que estaban referenciadas antes y ya no lo están; nunca borra URL externas ni medios al descartar.
...
- Pruebas y Guías comparten el banco web `users/{uid}/itemBank`, con búsqueda, filtro, borrado e inserción; los siete tipos de Prueba se convierten a actividades compatibles y los tipos exclusivos de Guías quedan deshabilitados al apuntar a Pruebas.
- Ambos editores ofrecen creación general con IA mediante `/api/generar-evaluacion` y token Firebase. El resultado se agrega como borrador revisable; adaptación PIE, simulación y calibración Bloom no forman parte de este flujo.
- Pruebas importa `.docx` hasta 10 MB mediante Office Open XML y aplica una heurística revisable; ambos módulos exportan `.docx`. PDF alumno/pauta también permite impresión con `UIPrintInteractionController`.
- El proyecto Xcode contiene la misma cantidad de referencias Swift que archivos Swift del target. La compilación, Simulator, dispositivo, Firebase real y apertura efectiva de PDF/DOCX quedan registrados para la siguiente sesión en Mac.
